...
from data_process.bev_utils import drawRotatedBox


def yaw_rotation_matrix(yaw):
    # Create a rotation object for yaw
...
def decode(hm_cen, cen_offset, direction, z_coor, dim, K=50):
...
def post_processing(detections, num_classes=2, down_ratio=4, peak_thresh=0.2):
    """
    ...
    for i in range(detections.shape[0]):
        ...
        for j in range(num_classes):
            ...
            top_preds[j] = np.concatenate([
                ...
                detections[i, inds, 5:6] / data_config.bound_size_y * data_config.BEV_WIDTH,
                detections[i, inds, 6:7] / data_config.bound_size_x * data_config.BEV_HEIGHT,
    ...
    for j in range(num_classes):
        if len(detections[j]) > 0:
            for det in detections[j]:
                ...
                drawRotatedBox(img, _x, _y, _w, _l, _yaw, (255, 0, 255))
    return img
    ...
    for cls_id in range(num_classes):
        if len(detections[cls_id]) > 0:
            for det in detections[cls_id]:
                # (scores-0:1, x-1:2, y-2:3, z-3:4, dim-4:7, yaw-7:8)
                _score, _x, _y, _z, _h, _w, _l, _yaw = det
                # print(_score, _x, _y, _z, _h, _w, _l, _yaw)
                _yaw = -_yaw
                x = _y / data_config.BEV_HEIGHT * data_config.bound_size_x + data_config.boundary['minX']
                y = _x / data_config.BEV_WIDTH * data_config.bound_size_y + data_config.boundary['minY']
                z = _z + data_config.boundary['minZ']
                w = _w / data_config.BEV_WIDTH * data_config.bound_size_y
                l = _l / data_config.BEV_HEIGHT * data_config.bound_size_x
...
def compute_3d_bbox_corners(x, y, z, w, l, h, yaw):
    # Create a rotation matrix from the yaw angle
    R = Rotation.from_euler('y', yaw).as_matrix()
    
    # Create an oriented bounding box
    box_3d = o3d.geometry.OrientedBoundingBox([x, y, z], R, [w, l, h])
    # box_3d.color = color  # Set the color (optional)

    # Get the 8 corners of the bounding box
    corners = np.asarray(box_3d.get_box_points())

    return corners



def compute_3d_bbox(x, y, z, w, l, h, yaw, color):
    # Create a rotation matrix from the yaw angle
    R = Rotation.from_euler('y', yaw).as_matrix()
    
    # Create an oriented bounding box
    box_3d = o3d.geometry.OrientedBoundingBox([x, y, z], R, [w, l, h])
    box_3d.color = color  # Set the color (option
    return box_3d